...
simulate_latent_change_multiple= function(
    ...
    latent.true.mean_x = 0,
    latent.true.mean_y = 0,
    observed.residual.var_x = 0.1,
    change.mean_x = 0.5,
    constant.change.var_x = 0.1,
    proportion.change_x = 0.2,
    ...
    change.mean_y = 0.1,
    constant.change.var_y = 0.1,
    proportion.change_y = 0.1,
  ...
  model_string <- paste0(model_string, "    cf_x1 ~", latent.true.mean_x, "*", 1,"\n")
  for(w in 2:waves){
...
# # Autoregressions
# for(w in 3:waves){
#   model_string <- paste0(model_string, "    ld_x", w, " ~ ", phi.x,"* ld_x", w-1, "\n")
# }

# Define the common factor for y
  for(w in 1:waves){
    model_string <- paste0(model_string, "    cf_y", w, " =~ 1*y", w, "\n")
  }
#
#   # Latent true score means (initial free, others = 0)
  model_string <- paste0(model_string, "    cf_y1 ~", latent.true.mean_y, "*", 1,"\n")
  for(w in 2:waves){
...
#   ## Combining models
#
#   # Coupling parameters, cross lag; do changes in one lead to changes in the other
  for(w in 2:waves){
    model_string <- paste0(model_string, "    ld_x", w, " ~ ", coupling_xy,"* cf_y", w-1, "\n")
    model_string <- paste0(model_string, "    ld_y", w, " ~ ", coupling_yx,"* cf_x", w-1, "\n")
  }
  # Specify the covariance between the general change factors, correlations and correlations of residuals
...
# %>%
#   reshape_long_sim_cr() -> tmp
#
# # Fit the OLS model and extract coefficients
# model_fit <- lm(I(y - ylag) ~   I(x - xlag), tmp)
# summary(model_fit)
