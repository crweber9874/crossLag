...
monteCarlo(
  ...
  waves = 5,
  proportion.change_x = seq(0, 0.7, by = 0.3),
  proportion.change_y = seq(0, 0.7, by = 0.3),
  change.mean_x = seq(0, 0.5, by = 0.25),
  change.mean_y = seq(0, 0.5, by = 0.25),
  #' Generate model syntax for a cross-lagged panel model, with or without random intercepts
#'
#' @param trials The number of trials for the Monte Carlo simulation.
#' @param waves The number of waves (time points) in the model.
...
monteCarlo(
  ...
  data_generation = "latent-change-xy",
  waves = 5,
  proportion.change_x = seq(-0.7, 0.7, by = 0.1),
  proportion.change_y = seq(-0.7, 0.7, by = 0.1),
  change.mean_x       = seq(-0.7, 0.7, by = 0.1),
  change.mean_y       = seq(-0.7, 0.7, by = 0.1),
  coupling_xy         = 0,
  coupling_yx         = 0,
) -> latenDat
...
monteCarlo(
  ...
  within_person_stability_x = seq(-0.7, 0.7, by = 0.1),
  within_person_stability_y = seq(-0.7, 0.7, by = 0.1),
  waves = 5,
) -> riDat
...
monteCarlo(
  ...
  model = "ols",
  data_generation = "ri-clpm",
  within_person_stability_x = seq(0, 0.7, by = 0.1),
  within_person_stability_y = seq(0, 0.7, by = 0.1),
  waves = 3,
) -> dat
